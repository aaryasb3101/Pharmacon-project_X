...
def run_one(smiles, drug_id, node_index, mode, depth, outdir, timeout):
    out_path = os.path.abspath(os.path.join(outdir, f"{node_index}_{drug_id}.csv"))
    if os.path.exists(out_path) and os.path.getsize(out_path) > 0:
        return out_path, "skipped (already done)"
0
    # BioTransformer install dir: the bioconda-installed 3.0 package, which
    # has a matched jar + config.json + database/ + supportfiles/ all
    # sitting together (confirmed via `find` — this is the only complete,
    # working install on disk; no standalone 4.0 bundle is publicly
    # available yet).
    BT_HOME = os.path.expanduser(
        "~/miniconda3/envs/biotransformer/share/biotransformer-3.0.20230403-0"
    )
    jar_path = None
    for file in os.listdir(BT_HOME):
        if file.lower().startswith("biotransformer") and file.endswith(".jar"):
            jar_path = os.path.join(BT_HOME, file)
            break

    if jar_path is None:
        print(f"ERROR: no BioTransformer jar found in {BT_HOME}")
        sys.exit(1)
    ...
    try:
        result = subprocess.run(
            ...
            cwd=BT_HOME,  # must run from here so BioTransformer finds config.json/database/supportfiles
        )
        if result.returncode != 0:
            return None, f"FAILED (code {result.returncode}): {result.stderr[-800:]}"
        if not os.path.exists(out_path):
            return None, (
                f"FAILED: no output file produced.\n"
